...
/**
 * Estrategia de agrupamiento por semana
 */
export class SemanaAgrupamientoStrategy implements AgrupamientoStrategy {
  buildGenerateSeries(fechaInicio: Date, fechaFin: Date): string {
    ...
    return `SELECT generate_series(date_trunc('week', '${inicio}'::date), date_trunc('week', '${fin}'::date), '1 week'::interval) AS periodo`;
  }
  ...
  buildSql(fechaInicio: Date, fechaFin: Date, tipoHabitacion?: string): string {
    ...
    return `
      ...
      LEFT JOIN "Reserva" r
        ON r.deleted = false
          AND r."fecha_inicio"::date <= (pb.periodo + INTERVAL '6 days')::date
